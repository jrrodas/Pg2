...
<?php
	...
	if(isset($_GET['id'])){
		...
		$buscar_id=$con->prepare('SELECT * FROM establecimientos_piedras_grandes WHERE id=:id LIMIT 1');
	...
	if(isset($_POST['guardar'])){
		...
		if(!empty($comercio) && !empty($cantidad) && !empty($tamano) && !empty($disponibilidad) && !empty($empleados) && !empty($formal) && !empty($informal)){
				...
				$consulta_update=$con->prepare(' UPDATE establecimientos_piedras_grandes SET  
					comercio=:comercio,
					cantidad=:cantidad,
					tamano=:tamano,
